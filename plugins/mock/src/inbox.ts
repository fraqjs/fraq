...
export interface MockMessageEventOverrides {
  time?: number;
  selfId?: number;
}
...
// In-memory message store that also answers the read-side API endpoints
// (get_message, get_history_messages, mark_message_as_read, and *_info lookups).
export class MockInbox {
  ...
  friendEvent(
    options: MockFriendMessageOptions,
    segments: readonly milky.IncomingSegment_ZodInput[] | readonly milky.IncomingSegment[],
    overrides?: MockMessageEventOverrides,
