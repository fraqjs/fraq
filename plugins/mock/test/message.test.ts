...
test('inmsg interpolates primitives as text', () => {
  const segments = inmsg`count: ${42} done: ${true}`;

  assert.deepEqual(segments, [
    {
      type: 'text',
      data: { text: 'count: 42 done: true' },
    },
  ]);
});

test('inseg factories apply sensible defaults', () => {
  assert.equal(inseg.mention(10001).data.name, 'user_10001');
  assert.equal(inseg.image().data.summary, '[image]');
  assert.equal(inseg.face(1).data.face_id, '1');
  assert.equal(inseg.face(1).data.is_large, false);
});
...
test('segments are cloned so later mutations do not leak into stored messages', () => {
  const inbox = createMockInbox();
  const segments = inmsg`hello`;
  const message = inbox.friend({ userId: 10001 }, segments);

  (segments[0] as { data: { text: string } }).data.text = 'mutated';

  assert.equal((message.segments[0] as { data: { text: string } }).data.text, 'hello');
});
...
test('reset clears stored conversations and entities', () => {
  const inbox = createMockInbox();
  const conversationKey = inbox.friendConversationKey(10001);
  inbox.friend({ userId: 10001 }, inmsg`one`);

  inbox.reset();

  assert.deepEqual(inbox.history(), []);
  assert.deepEqual(inbox.history(conversationKey), []);
  assert.equal(inbox.lastMessage(conversationKey), undefined);
});
