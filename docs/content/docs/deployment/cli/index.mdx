...
部分插件可能会要求安装额外的依赖包，版本声明方式遵循 [Semantic Versioning 规范](https://github.com/npm/node-semver#versions)。通常情况下，插件的 README 中会说明它们所依赖的额外依赖包及其版本要求。例如，在 `fraqjs/ai` 的配置中声明的 `sdk` 包也需要同时添加到 `additionalDependencies` 字段中。
...
plugins:
  fraqjs/ai:
    providers:
      ds:
        sdk: '@ai-sdk/deepseek' # 这里声明了 DeepSeek SDK 的依赖
        options:
...
这部分的依赖会被直接注入到生成的 `package.json` 中，并且在安装依赖时会被自动安装。
