...
{/* prettier-ignore */}
<Callout type="warn">
[`esnext.disposable.d.ts`](https://github.com/microsoft/TypeScript/blob/main/src/lib/esnext.disposable.d.ts) 也提供了一个 `Disposable` 接口，但其声明与 Fraq 的 `Disposable` 接口不同。`ESNext` 的 `Disposable` 接口声明如下：
