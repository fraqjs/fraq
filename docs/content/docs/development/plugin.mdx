...
- `fraq-plugin-a` 的 `AlphaService`：`a/AlphaService`
- `@acme/fraq-plugin-b` 的 `BetaService`：`acme/b/BetaService`
- `@fraqjs/plugin-hono` 的 `HonoService`：`fraqjs/hono/HonoService`
...
当定义了 `AlphaService` 并[发布](#准备发布)了插件 `a` 后，其他插件可以将 `fraq-plugin-a` 添加到它们的 `peerDependencies`：
