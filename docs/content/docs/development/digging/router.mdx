...
要了解 `Router` 的工作原理，首先要引入 token 的概念。在 `Context` 调用 `router.dispatch` 之后，`Router` 会将消息传入一个分词器（Tokenizer），然后对消息进行流式读取。每个候选分支的每种触发方式都会使用一个新的分词器，从消息开头独立尝试匹配。分词器也支持保存和恢复读取位置。
...
文本分隔符包括空格、制表符、换行符、回车符、垂直制表符、换页符、不换行空格（U+00A0）和全角空格（U+3000）。相邻文本消息段分别分词，不会合并。指令名、别名和指令组名必须匹配一个完整的文本 token，例如 `pingpong` 不会匹配指令 `ping`。
...
创建 `Context` 时也可以通过 [`routing.activationResolver`](./context.mdx#配置指令触发方式) 提供 resolver。`Context` 会把它设置到自己的 `router`，并让子 `Context` 继承同一个 resolver。
