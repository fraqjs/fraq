...
4. 指令处理函数通过 `Session` 对象来读取原始消息、回复消息，或对当前消息添加回应。
...
在前面的例子中，我们主要通过 `session.reply` 回复消息，也可以用 `session.reaction` 给当前消息添加回应。Milky 协议定义了丰富的 [API](https://milky.ntqqrev.org/api/system)，如果需要调用 `Session` 没有封装的能力，可以通过 `ctx.client` 直接访问它们：
