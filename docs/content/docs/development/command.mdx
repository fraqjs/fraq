...
可以使用 `.tag()` 给指令或模式添加标签。标签不会改变指令本身的解析规则，但可以供帮助生成、路由查询以及 [`routing.activation`](./digging/context.mdx#配置指令触发方式) 等功能使用：
...
例如，部署方可以通过 `match: { tag: 'admin' }` 让所有带有 `admin` 标签的指令在群聊中必须先提及机器人，或者必须使用 `/` 前缀。
...
自定义元信息可以在 `activationResolver`、`match.predicate` 或 [`router.branches`](./digging/router.mdx#白盒函数) 这类白盒函数中读取。插件中的 `ctx.router` 会自动带有插件名和当前 `Context` 名，因此插件作者通常不需要手动写入 `plugin` 或 `context` 元信息。
...
- `reaction`：给触发该指令的消息添加表情回应，仅在群聊场景下生效。`type` 可以是 `'face'` 或 `'emoji'`，`reactionId` 是对应的表情 ID。
