...
- `module`：日志来源的模块名称，默认为 `context`，但在插件里会自动设置为插件名称。
- `message`：日志内容字符串。
- `error`：可选字段，如果日志是由一个错误对象触发的，这里会包含这个错误对象。
- `timestamp`：日志生成的时间戳，通过 `Date.now()` 获取，单位是毫秒。
...
如果这个插件输出了一条日志，那么 `logHandler` 收到的 `module` 字段会是 `echo`，而不是默认的 `context`。这对于排查插件初始化顺序、依赖注入问题，或者区分不同功能模块的运行状态都很有帮助。
