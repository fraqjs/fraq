...
默认情况下，Fraq 不会把日志直接输出到终端。你可以通过 `ctx.logBus` 订阅所有日志：
...
`logBus` 在 root context 和所有子 context 之间共享。你也可以注册多个监听器，分别把日志输出到终端、文件或监控系统。

旧版本的 `logHandler` 选项仍然可以使用，Fraq 会自动把它注册到 `logBus`，但新代码建议直接使用事件订阅。
