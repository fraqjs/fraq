...
const options = ['Ras', 'Chiffon', 'Salt'];
const weights = [1, 3, 5];
const elem = options[ctx.random.weightedIndex(weights)];
// 根据权重随机选择一个元素，'Ras' 的概率为 1/9，'Chiffon' 的概率为 3/9，'Salt' 的概率为 5/9

// OR:
const elem = ctx.random.weightedPick(options, (index) => weights[index]);
```
...
此外，也支持保持 `sequence` 不变的情况下，使用新的种子来创建一个新的 `RandomService` 实例：

```typescript
const newRandom = ctx.random.withSeed(12345);
```
