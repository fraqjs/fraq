...
`router` 是 `Context` 实例的一个属性，我们用它来定义机器人将会响应哪些指令，并且这些指令的规则分别是什么样的。`ctx.router.command` 方法用来定义一个指令。它接受一个对象，其中包含：

- `name` 是指令的名称，这里是 `echo`，表示当用户发送的消息以 `echo` 开头时，这个指令就会被触发。
- `pattern` 是一个对象，定义了这个指令的参数规则。我们给 `echo` 指令定义了一个 `content` 参数，它是一个字符串类型的参数，表示用户在 `echo` 后面输入的文本内容。
- `execute` 是一个方法，当指令被触发时，这个方法会被调用。它接受两个参数：`session` 和一个包含指令参数的对象。
