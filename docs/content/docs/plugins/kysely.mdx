...
plugins:
  fraqjs/kysely:
    # SQLite 数据库的连接 URL
    # 默认值为 file:./fraq.db，表示在当前工作目录下创建一个名为 fraq.db 的 SQLite 数据库文件
...
启用自动 vacuum 后，插件会在迁移完成后立即整理一次数据库文件，并按配置的时间间隔重复执行。默认情况下该功能处于启用状态；如果不希望插件自动执行，可以设置 `autoVacuum.enabled` 为 `false`。也可以在需要时直接调用 `KyselyService` 的 `vacuum()` 方法。
