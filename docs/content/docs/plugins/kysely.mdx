...
- `sqliteUrl`：SQLite 数据库的连接 URL，默认为 `file:./fraq.db`，表示在当前工作目录下创建一个名为 `fraq.db` 的 SQLite 数据库文件。
...
definePlugin({
  ...
  apply(ctx) {
    // 使用 ctx.kysely 来访问 KyselyService
  },
});
```

此外，你还需要将 `kysely` 添加到 `devDependencies` 中，以便使用其中的类型定义。
...
Schema 本身只存在于类型系统，并且 Fraq 也无法比对不同版本的插件之间 Schema 是否发生了变化，因此 Fraq 选择让插件开发者来编写迁移脚本，以便在插件更新时执行这些脚本来同步数据库的结构。为此，`KyselyService` 提供了一个 `schemas` 属性，插件需要在 `apply` 阶段调用其中的方法来注册 Schema 和迁移脚本。
...
`KyselyService` 对迁移状态的记录完全基于你提供的脚本名。假设一个数据库已经执行过脚本 `001_create_person_table`，那么在插件更新后，`KyselyService` 会从 `001` 的下一个脚本开始执行迁移；在这个例子中，它会执行 `002_add_person_nickname`。
...
```typescript
const person = await ctx.kysely.db
...
```typescript
await ctx.kysely.db.transaction().execute(async (trx) => {
