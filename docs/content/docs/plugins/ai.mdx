...
`@fraqjs/plugin-ai` 提供了配置模型提供商和 API Key 的统一入口，方便基于 [Vercel AI SDK](https://ai-sdk.dev/) 的 AI 插件开发者访问用户开发者配置的语言模型和生图模型。
...
plugins:
  fraqjs/ai:
    ...
    providers:
      ...
      deepseek:
        # 目前支持自动配置的 SDK 包括：
        # - `@ai-sdk/alibaba` (Qwen 系列)
        # - `@ai-sdk/anthropic` (Claude 系列, /v1/messages 风格 API)
        # - `@ai-sdk/bytedance` (Seedream 系列)
        # - `@ai-sdk/deepseek` (DeepSeek 系列)
        # - `@ai-sdk/google` (Gemini 系列)
        # - `@ai-sdk/moonshotai` (Kimi 系列)
        # - `@ai-sdk/openai` (GPT 系列, /v1/responses 风格 API)
        # - `@ai-sdk/openai-compatible` (/v1/chat/completions 风格 API)
        # - `@ai-sdk/xai` (Grok 系列)
        sdk: '@ai-sdk/deepseek'
        # 这里用于提供传入 SDK 的配置项
        # 基本的配置项有：
...
Vercel 的 `ai` 包提供了丰富的函数来调用模型，只需要传入模型实例和相应的参数即可。`@fraqjs/plugin-ai` 将 `ai` 包的所有功能导出为 `ai` 变量，只需要在代码中加入：

```typescript
import { ai } from '@fraqjs/plugin-ai';
```

即可使用 `ai` 包中的函数。下面是一些简单的例子，更多用法请参考 [Vercel AI SDK 的文档](https://ai-sdk.dev/docs)。
...
const { toolResults } = await ai.generateText({
  model: ctx.ai.model(),
  prompt: '东京现在的天气怎么样？',
  tools: {
    weather: ai.tool({
...
- `resources`：资源键到临时 URL 的映射，例如 `{ image1: { url: 'https://...' } }`。其中 `image`、`record`、`video` 类型的消息段会分别以 `image1`、`record1`、`video1` 等递增编号作为键。
- `files`：资源键到文件数据的映射，对应 `file` 类型的消息段。
- `forward`：资源键到转发消息的映射，对应 `forward` 类型的消息段。

`<message>` 根元素上会附带 `scene`、`peer_id`、`seq`、`sender_id`、`time` 等属性；根据消息场景，还会包含 `<friend>`、`<group>`、`<group_member>` 等子元素，描述发送者与会话的上下文信息。

### 转换多条消息

`xmlifyThread` 用于将多条消息合并为一个 `<thread>` 元素，适合将一段对话历史整体提供给模型。它会复用同一个资源编号空间，确保所有消息中的 `image`、`record`、`video`、`file`、`forward` 键互不冲突：
...
在 `xmlifyThread` 中，每个 `<message>` 元素只保留 `seq`、`sender_id`、`time` 属性，场景与对端信息统一由外层 `<thread>` 携带。

### 配置项

`xmlify` 和 `xmlifyThread` 的第三个参数是一个可选的 `XmlifyOptions` 对象，支持以下配置项：

- `maxForwardDepth`：合并转发消息的展开深度，默认为 `0`。为 `0` 时，合并转发不会被展开，仅输出一个 `id` 及占位文本；大于 `0` 时，会通过 `ctx.client.get_forwarded_messages` 拉取转发内容，并递归展开到指定深度。超出 `maxForwardDepth` 的嵌套层级会以 `(Too deeply nested)` 占位，同样包含一个 `id`，避免无限制地拉取深层转发内容。
