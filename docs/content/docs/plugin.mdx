...
在 Fraq 中，插件（Plugin）是一个可选、可复用的模块化组件，用于封装特定的功能或一组相关的功能。插件可以包含命令、事件处理器等，并且可以被其他插件依赖和调用，只要声明并提供了对应的服务（Service）。下面我们来介绍一下如何定义和使用插件。
...
我们可以看到，插件是一个对象，它有一个 `apply` 方法，这个方法会在插件被加载时被调用，并且会传入一个 `Context` 对象作为参数。在 `apply` 方法中，我们可以像之前一样定义命令、事件处理器等。
