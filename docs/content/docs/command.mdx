...
这段代码包含了三个要素：命令叫什么、需要什么参数、以及收到这个命令时要执行什么操作。“快速开始” 一节已经对上述代码进行了简要的拆解，下面我们将更深入地介绍 Fraq 中的指令系统的设计与使用方法。
...
ctx.router
  .group('parent')
  .command('child1', { arg1: param.str() }, async (session, { arg1 }) => {
    // Handle child1 command
  })
  .command('child2', { arg2: param.num() }, async (session, { arg2 }) => {
    // Handle child2 command
  });
```

这样，我们就定义了一个名为 `parent` 的指令组，其中包含两个子命令 `child1` 和 `child2`。每个子命令都有自己的参数和处理函数。当用户输入 `parent child1 <arg1>` 时，`child1` 的处理函数将被调用；当用户输入 `parent child2 <arg2>` 时，`child2` 的处理函数将被调用。
...
`ctx.router` 提供了一个名为 `rawPattern` 的方法，它允许我们直接使用一个参数模式对象从头开始匹配用户输入，而不是定义命令名称和参数类型，通过检测用户输入是否符合这个模式来决定是否调用对应的处理函数。例如，如果我们需要用户先引用（回复）一条消息，再输入指令，我们可以定义如下的 Raw Pattern：
...
这样就可以通过 `reply` 来拿到用户引用的消息段信息，通过 `content` 来拿到用户输入的剩余文本信息了。这比一般的命令定义更灵活，因为它不要求用户输入一个特定的命令名称，只要输入符合模式就可以了。

## 命令重载

模式匹配还可以用于命令重载。在上面的例子中，我们一直都假设每个指令都只有一种参数模式，但实际上有时候我们可能希望一个指令能够接受多种不同的参数模式。例如，我们可能希望 `echo` 指令既能接受一个字符串参数，也能接受一个图片消息段参数；一个更典型的例子是游戏《Minecraft》中的 `tp` 指令，它既可以接受玩家名称作为参数，也可以接受坐标作为参数，例如：
...
ctx.router
  ...
  .rawPattern(
    ...
    async (session, { player, x, y, z }) => {
      // Handle tp with player and coordinates
    },
  )
  .rawPattern(
    { player1: param.str(), player2: param.str() },
    async (session, { player1, player2 }) => {
      // Handle tp with player to player
    },
  );
```

定义命令重载时需要格外小心，确保不同的参数模式之间没有歧义，否则可能会导致某些输入无法正确匹配到对应的处理函数。
...
adminRouter.command('admincmd', { arg: param.str() }, async (session, { arg }) => {
...
<Callout>
  对于插件开发者，非必要情况下不应该主动调用 `ctx.router.filter` 来控制命令的触发权限，因为这会导致指令的可见性和触发条件不能被插件部署方所预期。更好的做法是将过滤命令的任务交给插件部署方，由插件部署方使用 `ctx.fork` 来创建新的 `Context` 实例，并在新的实例上安装插件来实现指令的权限控制。
</Callout>
